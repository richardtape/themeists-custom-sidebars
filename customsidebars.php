<?php
/*
Plugin Name: Themeists Custom Sidebars
Plugin URI: #
Description: Allows to create your own widgetized areas and custom sidebars, and select what sidebars to use for each post or page. A fork of the Custom sidebars plugin by Javier Marquez.
Version: 1.0
Author: Themeists
Author URI: #
License: GPL2
...
if( !class_exists( 'ThemeistsCustomSidebars' ) ):

class ThemeistsCustomSidebars
{
	...
	var $option_name = "themeists_custom_sidebars";
	...
	var $sidebar_prefix = 'tcs-';
	var $postmeta_key = '_tcs_replacements';
	var $cap_required = '';
	var $ignore_post_types = array();
	var $options = array();
	...
	function ThemeistsCustomSidebars()
	{

		$this->cap_required = apply_filters( 'themeists_sidebars_cap_required', 'switch_themes' );
		$this->ignore_post_types = apply_filters( 'themeists_sidebars_ignore_post_types', array('attachment', 'revision', 'nav_menu_item', 'pt-widget') );
	...
	function getThemeistsCustomSidebars()
	{
	...
	function registerThemeistsCustomSidebars()
	{
	...
	function deleteSidebar()
	{
		...
		$custom = $this->getThemeistsCustomSidebars();
	...
	function createPage()
	{
		...
		if(!empty($_POST)){
			...
			if($_GET['p']=='edit' && !empty($_GET['id'])){
				$ThemeistsCustomSidebars = $this->getThemeistsCustomSidebars();
				if(! $sb = $this->getSidebar($_GET['id'], $ThemeistsCustomSidebars))
					return new WP_Error('cscantdelete', __('You do not have permission to delete sidebars','custom-sidebars'));
	...
	{
		$page = add_submenu_page('themes.php', __('Custom sidebars','custom-sidebars'), __('Custom sidebars','custom-sidebars'), $this->cap_required, 'ThemeistsCustomSidebars', array($this, 'createPage'));
	...
	function addStyles($hook){
            ...
            if( 'widgets.php' == $hook || 'appearance_page_ThemeistsCustomSidebars' == $hook){
	...
	function addMetaBox()
	{
		...
		if($post_type && !(array_search($post_type, $this->ignore_post_types))){
			...
			if($post_type_object->publicly_queryable || $post_type_object->public) 
				add_meta_box('ThemeistsCustomSidebars-mb', 'Sidebars', array($this,'printMetabox'), $post_type, 'side');
		}
	}
	...
	function updateSidebar()
	{
		...
		$sidebars = $this->getThemeistsCustomSidebars();
	...
	function getEmptyWidget()
	{
		return array(
			...
			'id' => 'csemptywidget',
			'callback' => array(new ThemeistsCustomSidebarsEmptyPlugin(), 'display_callback'),
			'params' => array(array('number' => 2)),
			'classname' => 'ThemeistsCustomSidebarsEmptyPlugin',
	...
	{
            $id = trim($_POST['cs_id']);
            $sidebar = $this->getSidebar($id, $this->getThemeistsCustomSidebars());
...
if(!isset($plugin_sidebars)){
	$plugin_sidebars = new ThemeistsCustomSidebars();	
	add_action( 'widgets_init', array($plugin_sidebars,'registerThemeistsCustomSidebars') );
...
if(! class_exists('ThemeistsCustomSidebarsEmptyPlugin')){
class ThemeistsCustomSidebarsEmptyPlugin extends WP_Widget {
	function ThemeistsCustomSidebarsEmptyPlugin() {
		parent::WP_Widget(false, $name = 'ThemeistsCustomSidebarsEmptyPlugin');
	}
